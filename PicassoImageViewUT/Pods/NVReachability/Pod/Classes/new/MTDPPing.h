...
//  NVReachability
//
//  Created by xiangnan.yang on 2017/12/7.
//
...
@protocol MTDPPingDelegate;

/*! Controls the IP address version used by SimplePing instances.
 */

typedef NS_ENUM(NSInteger, MTDPPingAddressStyle) {
    MTDPPingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    MTDPPingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    MTDPPingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@property (nonatomic, weak, readwrite, nullable) id<MTDPPingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) MTDPPingAddressStyle addressStyle;
...
- (NSData *)sendPingWithData:(nullable NSData *)data;
...
@protocol MTDPPingDelegate <NSObject>
...
- (void)simplePing:(MTDPPing *)pinger didStartWithAddress:(NSData *)address;
...
- (void)simplePing:(MTDPPing *)pinger didFailWithError:(NSError *)error;
...
- (void)simplePing:(MTDPPing *)pinger didSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;
...
- (void)simplePing:(MTDPPing *)pinger didFailToSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber error:(NSError *)error;
...
- (void)simplePing:(MTDPPing *)pinger didReceivePingResponsePacket:(NSData *)packet timeToLive:(NSInteger)timeToLive sequenceNumber:(uint16_t)sequenceNumber timeElapsed:(NSTimeInterval)timeElapsed;
...
- (void)simplePing:(MTDPPing *)pinger didReceiveUnexpectedPacket:(NSData *)packet;
