...
function pLogInfo(msg) {
    ...
    if (typeof msg === 'string') {
        picassoLog(msg, 2 /* PLogTagInfo */);
    }
    else {
        picassoLog(JSON.stringify(msg), 2 /* PLogTagInfo */);
    }
}
...
var PCSB = (function () {
    ...
    PCSB.prototype.sendMsg = function (module, method, args) {
        ...
        var fail = function (result) {
            pLogInfo('调用失败 ' + module + '.' + method + ",result:" + JSON.stringify(result));
            _fail && _fail.call(self, result);
        };
        var success = function (result) {
            pLogInfo('调用成功 ' + module + '.' + method + ",result:" + JSON.stringify(result));
            _success && _success.call(self, result);
        };
        var handle = function (result) {
            pLogInfo('回调 ' + module + '.' + method + ",result:" + JSON.stringify(result));
    ...
    PCSB.prototype.doSend = function (module, method, args, callback) {
        ...
        pLogInfo('in Page' + this.hostId + ' send message ' + module + '.' + method + ",args=" + JSON.stringify(args) + ";  callbackId:" + callbackId);
    ...
    PCSB.prototype.callback = function (callbackId, ret, data) {
        if (callbackId && this.callbacks[callbackId]) {
            ...
            if (ret && (ret.status === "success" || ret.status === "fail")) {
                delete this.callbacks[callbackId];
            }
            return rv;
        }
        else {
            pLogInfo("找不到对应callback,callbackId=" + callbackId);
        }
    };
    return PCSB;
}());
...
var callback = function (hostId, callbackId, ret, data) {
    ...
    if (!temp) {
        pLogInfo('callback: cannot find context for' + hostId);
        return;
    }
...
function prepareBridgeModule(hostId, moduleName) {
    ...
    pLogInfo('Cannot find PicassoController for:' + moduleName);
    return prepareContext(hostId, '', '');
}
function prepareContext(hostId, options, data) {
    if (pContexts[hostId]) {
