...
int open_mmap_file_clogan(char *_filepath, unsigned char **buffer, unsigned char **cache) {
    ...
    if (NULL == _filepath || 0 == strnlen(_filepath, 128)) {
        ...
        if(fd != -1){ //保护
            int isFileOk = 0;
            FILE *file = fopen(_filepath, "rb+"); //先判断文件是否有值,再mmap内存隐射
            if (NULL != file) {
            ...
            if(isNeedCheck){ //加强保护,对隐射的文件要有一个适合长度的文件
    ...
    if (NULL != tempData) {
        ...
        *cache = tempData;
        if (back != LOGAN_MMAP_MMAP) {
            *buffer = tempData;
            back = LOGAN_MMAP_MEMORY; //如果文件打开失败、如果mmap隐射失败,走内存缓存
        }
    } else {
        if (back != LOGAN_MMAP_MMAP)
