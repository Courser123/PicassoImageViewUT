...
static char *_mmap_file_path = NULL;//mmap文件路劲 (不释放)
...
static void (*cat_upload)(char *, int) = NULL;

/**
 * 回调函数,返回cmd 和 status,用于打点记录
 * @param cmd
 * @param status
 */
void uploadcat_clogan(char *cmd, int status) {
    if (NULL != cat_upload) {
        cat_upload(cmd, status);
    }
}
...
void init_encrypt_key_clogan(cLogan_Model *logan_model) {
    aes_infalte_vi_clogan(logan_model->aes_vi);
}


void write_mmapdata_clogan(char *path, unsigned char *temp) {
...
void read_mmapdata_clogan(const char *pathdirs) {
                ...
                if (LOGAN_MMAP_TAIL_PROTOCOL == i) {
                    ...
                    if (NULL != cjson) {
                        ...
                        if ((NULL != dir_str && cJSON_Number == dir_str->type &&
                            ...
                            printf_clogan("read_mmapdata_clogan > dir , path and version : %s || %s || %lf\n", pathdirs,
                                          path_str->valuestring, dir_str->valuedouble);

                            size_t dir_len = strlen(pathdirs);
                            ...
                            memcpy(file_path, pathdirs, dir_len);
                            strcat(file_path, path_str->valuestring);
                            temp++;
                            write_mmapdata_clogan(file_path, temp);
                        }
                        cJSON_Delete(cjson);
                    }
                }
...
int clogan_init(const char *cachedirs, const char *pathdirs, int max_file, const char *encrypt_key_factor,
    ...
    if (is_init_ok || NULL == cachedirs || NULL == pathdirs
        || strnlen(pathdirs, 11) == 0 || NULL == encrypt_key_factor || strnlen(cachedirs, 11) == 0) {
        back = CLOGAN_INIT_FAIL_HEADER;
        uploadcat_clogan(CLGOAN_INIT_STATUS, back);
        return back;
    }
    ...
    back = aes_init_key_clogan(encrypt_key_factor, (size_t) encrypt_key_len); //根据因子初始化aes的加密key
    if (back != 0) {
        return back;
    }

    size_t path1 = strlen(cachedirs);
    size_t path2 = strlen(LOGAN_CACHE_DIR);
    size_t path3 = strlen(LOGAN_CACHE_FILE);
    size_t path4 = strlen(LOGAN_DIVIDE_SYMBOL);

    int isAddDivede = 0;
    char d = *(cachedirs + path1 - 1);
    if (d != '/') {
        isAddDivede = 1;
    }
    ...
    size_t dirLength = strlen(pathdirs);

    isAddDivede = 0;
    d = *(pathdirs + dirLength - 1);
    if (d != '/') {
        isAddDivede = 1;
    }
    ...
    memset(dirs, 0, total);
    memcpy(dirs, pathdirs, dirLength);
    if (isAddDivede)
    ...
    if (is_init_ok) {
        ...
        if (flag == LOGAN_MMAP_MMAP) //MMAP的缓存模式,从缓存的MMAP中读取数据
            read_mmapdata_clogan(_dir_path);
...
int clogan_open(const char *pathname) {
    ...
    if (!is_init_ok) {
        ...
        uploadcat_clogan(CLOGAN_OPEN_STATUS, back);
        return back;
    }

    is_open_ok = 0;
    if (NULL == pathname || 0 == strnlen(pathname, 128) || NULL == _logan_buffer || NULL == _dir_path ||
    ...
    aes_init_iv_clogan(pathname, strlen(pathname)); //根据文件因子+规则生成IV(动态IV)
...
//对空的文件插入一行头文件做标示
void insert_header_file(cLogan_Model *loganModel) {
    char *log = "clogan header";
    int flag = 1;
    long long sync_time = get_system_current_clogan();
    long long local_time = get_system_current_clogan();
    char *thread_name = "clogan";
    char *extra_tag = NULL;
    long long thread_id = 1;
    int ismain = 1;
    Construct_Data_cLogan *data = construct_json_data_clogan(log, flag, sync_time, local_time, thread_name, thread_id,
                                                             ismain, extra_tag);
    if (NULL == data) {
        return;
    }
    cLogan_Model temp_model; //零时的clogan_model
    int status_header = 1;
    ...
    if (temp_model.isMalloc_zlib) {
        free(temp_model.strm);
        temp_model.isMalloc_zlib = 0;
    }
...
void clogan_write2(char *data, int length) {
    if (NULL != logan_model && logan_model->isOk) {
        clogan_zlib_compress(logan_model, data, length);
        update_length_clogan(logan_model); //有数据操作,要更新数据长度到缓存中
        int is_gzip_end = 0;

        if (!logan_model->file_len || logan_model->content_len >= LOGAN_MAX_GZIP_UTIL) { //是否一个压缩单元结束
            clogan_zlib_end_compress(logan_model);
            is_gzip_end = 1;
            update_length_clogan(logan_model);
        }
...
/**
 ...
 @param flag 日志类型 (int)
 @param log 日志内容 (char*)
 @param sync_time 同步过的日志发生时间，形如1502100065601 (long long)
...
int clogan_write(int flag, char *log, long long sync_time, long long local_time, char *thread_name, long long thread_id,
                 int ismain) {
    return clogan_write_tag(flag, log, sync_time, local_time, thread_name, thread_id, ismain, NULL);
}
...
clogan_write_tag(int flag, char *log, long long sync_time, long long local_time, char *thread_name, long long thread_id,
    ...
    if (is_file_exist_clogan(logan_model->file_path)) {
        if (logan_model->file_len > max_file_len) {
            ...
            uploadcat_clogan(CLOGAN_WRITE_STATUS, back);
            return back;
        }
    } else {
        if (logan_model->file_stream_type == LOGAN_FILE_OPEN) {
...
//                break;
//            case 2 :
//                sync_time = va_arg(v1, long long);
//                break;
//            case 3 :
//                local_time = va_arg(v1, long long);
//                break;
//            case 4 :
//                thread_name = va_arg(v1, char *);
//                break;
//            case 5:
//                thread_id = va_arg(v1, long long);
//                break;
//            case 6:
//                ismain = va_arg(v1, int);
//                break;
//            case 7:
//                extra_tag = va_arg(v1, char*);
//                isOver = 1;
//                break;
//        }
//        i++;
...
//
//    if (!isOver) {
//        printf_clogan("clogan_write > read param error\n");
//        back = CLOGAN_WRITE_FAIL_PARAM;
//        uploadcat_clogan(CLOGAN_WRITE_STATUS, back);
//        return back;
//    }
//
...
int clogan_flush() {
    ...
    if (!is_init_ok || NULL == logan_model) {
        uploadcat_clogan(CLOGAN_FLUSH_STATUS, back);
        return back;
    }
    write_flush_clogan();
